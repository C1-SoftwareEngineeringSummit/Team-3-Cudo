...
const rnDiffPurgeRawDiffsUrl = 'https://raw.githubusercontent.com/react-native-community/rn-diff-purge/diffs/diffs';
...
const getPatch = async (currentVersion, newVersion, config) => {
  ...
  try {
    patch = await (0, _fetch.fetch)(`${rnDiffPurgeRawDiffsUrl}/${currentVersion}..${newVersion}.diff`);
  } catch (error) {
...
const applyPatch = async (currentVersion, newVersion, tmpPatchFile) => {
  ...
  try {
    try {
      ...
      _cliTools().logger.info(`Applying diff (excluding: ${filesToExclude.join(', ')})...`);
    } finally {
      const excludes = filesToExclude.map(e => `--exclude=${_path().default.join(relativePathFromRoot, e)}`);
    ...
    if (error.stderr) {
      _cliTools().logger.log(`${_chalk().default.dim(error.stderr.trim())}`);
    }

    _cliTools().logger.error('Automatically applying diff failed');
...
async function upgrade(argv, ctx, args) {
  ...
  try {
    ...
    if (!patchSuccess) {
      if (stdout) {
        _cliTools().logger.warn('Continuing after failure. Most of the files are upgraded but you will need to deal with some conflicts manually');
...
• Comparison between versions: ${_chalk().default.underline.dim(`${rnDiffPurgeUrl}/compare/release/${currentVersion}..release/${newVersion}`)}
• Git diff: ${_chalk().default.underline.dim(`${rnDiffPurgeRawDiffsUrl}/${currentVersion}..${newVersion}.diff`)}`);
